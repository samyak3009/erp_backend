...
def emp_deductions_report(request):
	if checkpermission(request,[rolesCheck.ROLE_ACCOUNTS])== statusCodes.STATUS_SUCCESS:
		if requestMethod.GET_REQUEST(request):
			...
			const_ded_name = AccountsDropdown.objects.filter(sno__in=const_ded_list).values('value','sno')
			var_ded_name = AccountsDropdown.objects.filter(sno__in=variable_ded_list).values('value','sno')
