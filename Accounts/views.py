            ...
            if check == 200:
                if request.method == 'GET':
                    ...
                    if len(qry_check1) > 0:
                        ...
                        total_epf = calculate_epf(emp_id, session, [0, 1], gross_payable, month)
                        ...
                        loss = calculate_loss_deductions(emp_id, session, gross_payable, const_ded, month)
            ...
            if check == 200:
                if request.method == 'GET':
                    if request.GET['request_type'] == 'form_data':
                        ...
                        session = getCurrentSession(month_date)
                ...
                if request.method == 'GET':
                    ...
                    if len(qry_check_leave) > 0:
                        for c in qry_check_leave:
                            ...
                            if c['fromdate'].date() == date or c['todate'].date() == date:
...
def getCurrentSession_BySessionName(session_name):
    qry_session = AccountsSession.objects.filter(session=session_name).values('id').order_by('-id')[:1]
    if len(qry_session) == 0:
        return -1
    return qry_session[0]['id']
...
def calculate_increment_days_arrear(emp_id, session, month, working_days_li):
    ...
    new_salary_ids = []
    for n in new_salary:
        new_salary_ids.append(n['Ing_Id'])

    for sal in qry_sal_ing:
        if sal['id'] not in new_salary_ids:
            sal['gross_value'] = 0
        else:
            sal['gross_value'] = new_salary[new_salary_ids.index(sal['id'])]['gross_value']
    ...
    old_gross_li = []
    for sal in qry_sal_ing:
        old_gross = sal['gross_value']

        diff_value = 0
        for w in working_days_li:
            q_old_det = MonthlyPayable_detail.objects.filter(Month=w['month'], Emp_Id=emp_id, session=w['session'], Ing_Id__Ingredients__value=sal['Ingredients__value']).values('gross_value', 'Ing_Id__Ingredients__value')
            if len(q_old_det) > 0:
                old_gross = q_old_det[0]['gross_value']

            try:
                diff_value += ((round((w['working_days'] * sal['gross_value']) / w['days']) - round((w['working_days'] * old_gross) / w['days'])))
            except:
                diff_value = 0

        new_salary2.append({'gross_value': sal['gross_value'], 'Ing_Id': sal['id'], 'value': sal['Ingredients__value']})
        old_salary.append({'gross_value': old_gross, 'Ing_Id': sal['id'], 'value': sal['Ingredients__value']})
        difference.append({'diff_value': diff_value, 'Ing_Id': sal['id'], 'value': sal['Ingredients__value']})

    data = {'old_payable': old_salary, 'new_payable': new_salary2, 'diff_payable': difference}
    return data
...
def calculate_income_tax_sum(emp_id, session, working_days, actual_days, gross_payable_data, salary_month, payable_da_arrear, payable_inc_arrear, payable_days_arrear, additional_arrear, payable_sign_arrear):
    ...
    q_gen = DaysGenerateLog.objects.filter(sessionid=session, month=salary_month, acc_sal_lock='Y').values('id')
    if len(q_gen) > 0:
        flg_locked = True
    else:
        flg_locked = False
    ...
    for i in range(n):
        ...
        qry_mon = MonthlyPayable_detail.objects.filter(Emp_Id=emp_id, Ing_Id=gross_payable_data[i]['Ing_Id'], session=session, Month__in=months_list).values('payable_value', 'gross_value', 'Month').order_by('id')
        ...
        qry_mon = MonthlyArrear_detail.objects.filter(Emp_Id=emp_id, Ing_Id=gross_payable_data[i]['Ing_Id'], arrear_type="L", session=session, Month__in=months_list).values('value', 'Month').order_by('id')
        ...
        qry_mon = MonthlyArrear_detail.objects.filter(Emp_Id=emp_id, Ing_Id=gross_payable_data[i]['Ing_Id'], arrear_type="I", session=session, Month__in=months_list).values('value', 'Month').order_by('id')
        ...
        qry_mon = MonthlyArrear_detail.objects.filter(Emp_Id=emp_id, Ing_Id=gross_payable_data[i]['Ing_Id'], arrear_type="S", session=session, Month__in=months_list).values('value', 'Month').order_by('id')
        ...
        if comp_name == "DA":
            ...
            qry_mon = MonthlyArrear_detail.objects.filter(Emp_Id=emp_id, Ing_Id=gross_payable_data[i]['Ing_Id'], arrear_type="DA", session=session, Month__in=months_list).values('value', 'Month').order_by('id')
        ...
        if comp_name == "OTHERS":
            ...
            qry_mon = MonthlyArrear_detail.objects.filter(Emp_Id=emp_id, Ing_Id=gross_payable_data[i]['Ing_Id'], arrear_type="A", session=session, Month__in=months_list).values('value', 'Month').order_by('id')
...
def hra_exemption_formula1(emp_id, session, working_days, actual_days, month):
    ...
    value1 = 0
    qry_hra = MonthlyPayable_detail.objects.filter(Ing_Id__Ingredients__value__contains="HRA", Emp_Id=emp_id, session=session, Month__in=months_list).values('payable_value')
    for q in qry_hra:
        value1 += q['payable_value']

    qry_hra = MonthlyArrear_detail.objects.filter(Ing_Id__Ingredients__value__contains="HRA", Emp_Id=emp_id, session=session, Month__in=months_list).values('payable_value')
    for q in qry_hra:
        value1 += q['payable_value']
...
def calculate_loss_deductions(emp_id, session, payable, const_ded, month):
    print(emp_id, session, payable, const_ded, month, 'vrinda')
    session_id = session
    final_value = 0
    taxstatus_li = [0]
    ...
    for l in loss:
        ...
        if l['value'] == '80D':
            ...
            med_data = calculate_mediclaim(emp_id, session, taxstatus_li, payable, month)
            ...
            if l['value'] == '80C':
                epf_data = calculate_epf(emp_id, session, taxstatus_li, payable, month)
...
def calculate_mediclaim(emp_id, session, taxstatus_li, payble, sal_month):
    ################ mediclaim constant deduction #######################################
    ...
    for a in query:
        ...
        q_find = MonthlyDeductionValue.objects.filter(Emp_Id=emp_id, session=session, month__in=months_list, deduction_id__variableDeduction=None, deduction_id__constantDeduction__DeductionName__value='MEDICLAIM').values('value', 'month')
        for qf in q_find:
            while mon2 != qf['month']:
        ...
        while cnt > 0:
            if sal_month >= 4 and sal_month <= a['constantDeduction__creditdate'].month and a['constantDeduction__creditdate'].month == mon2:
                mediclaim_break_up.append({'month': calendar.month_name[mon2], 'value': med_val_t})
                final_value_m += med_val_t
            else:
                mediclaim_break_up.append({'month': calendar.month_name[mon2], 'value': 0})
...
def calculate_epf(emp_id, session, taxstatus_li, payable, month):
    flag_e = 0
    ...
    for a in query:
        ...
        q_find = MonthlyDeductionValue.objects.filter(deduction_id__constantDeduction__DeductionName__value='EPF', Emp_Id=emp_id, session=session, month__in=months_list).values('value', 'month')
        if len(q_find) > 0:
...
def calculate_monthly_income_tax(final_income_tax, session, emp_id, month):
    ...
    qry_tax = Income_Tax_Paid.objects.filter(emp_id=emp_id, session=session, month__in=months_list).values('monthly_tax_paid', 'month').order_by('id')
    for q in qry_tax:
        while mon != q['month']:
            ...
            if check == 200:
                if request.method == 'GET':
                    ...
                    if dept == 'ALL':
                        ...
                        emps = EmployeePayableDays.objects.filter(emp_id='7035').filter(emp_id__dept__in=list(query), session=session, month=fr_month).values('working_days', 'leave', 'holidays', 'total_days', 'emp_id__name', 'emp_id', 'emp_id__dept__value', 'emp_id__desg__value')
                    ...
                    if len(emps) == 0:
                        ...
                        for e in emps:
                            ...
                            loss = calculate_loss_deductions(emp_id, session, payable, constant_deductions, month)
                            ...
                            variation = calculate_variation(emp_id, month, gross_value, session)
                    ...
                    if fr_month > month:
                        ...
                        check_date = q_ses[0]['current_sal_month'] == str(tdate).split("-")[1]
...
def stored_arrear(emp_id, month, session):
    ...
    salary_ids = []
    for n in q_s:
        salary_ids.append(n['Ing_Id'])

    for sal in qry_sal_ing:
        if sal['id'] not in salary_ids:
            sign_data.append({'Ing_Id': sal['id'], 'payable_value': 0, 'value': sal['Ingredients__value']})
        else:
            val = q_s[salary_ids.index(sal['id'])]['value']
            sign_data.append({'Ing_Id': sal['id'], 'payable_value': val, 'value': sal['Ingredients__value']})
            v += val
    ...
    for sal in qry_sal_ing:
        if sal['id'] not in salary_ids:
            leave_data.append({'Ing_Id': sal['id'], 'payable_value': 0, 'value': sal['Ingredients__value']})
        else:
            val = q_l[salary_ids.index(sal['id'])]['value']
            leave_data.append({'Ing_Id': sal['id'], 'payable_value': val, 'value': sal['Ingredients__value']})
            v += val
    ...
    for sal in qry_sal_ing:
        if sal['id'] not in salary_ids:
            inc_data.append({'Ing_Id': sal['id'], 'diff_value': 0, 'value': sal['Ingredients__value']})
        else:
            val = q_i[salary_ids.index(sal['id'])]['value']
            inc_data.append({'Ing_Id': sal['id'], 'diff_value': val, 'value': sal['Ingredients__value']})
            v += val
...
def calculate_variation(emp_id, salary_month, gross_value, session):

    month2 = salary_month - 1
    if month2 < 0:
        month2 = 12
    qry_last_month = MonthlyPayable_detail.objects.filter(Emp_Id=emp_id, Month=month2, session=session).aggregate(sum=Sum('gross_value'))
...
def category_sheet(row, col, qry_salary_comp, q_emps, worksheet, q_const_ded, q_var_ded, cell_format, cell_format2, cell_format3, merge_format, session, month, qry_salary_comp_ids):
    ...
    for e in q_emps:
        ...
        if month == curr_mon:
            ...
            loss = calculate_loss_deductions(emp_id, session, gross_comp, constant_deductions, month)
...
def modified_category_sheet(qry_salary_comp, q_emps, q_const_ded, q_var_ded, session, month, qry_salary_comp_ids):
    ...
    for e in q_emps:
        ...
        if month == curr_mon:
            ...
            payable_inc_arrear = calculate_increment_days_arrear(emp_id, session, month, calculate_increment_days(emp_id, session, month))
            print(payable_inc_arrear, 'payable_inc_arrear')
            n = len(payable_inc_arrear['diff_payable'])
        ...
        final_sheet_array['variation'] = calculate_variation(emp_id, month, total, session)
        ################# payable ######################################
...
def form_16(request):
    if checkpermission(request,[337]) == statusCodes.STATUS_SUCCESS:
        if(requestMethod.GET_REQUEST(request)):
            if(requestType.custom_request_type(request.GET, 'get_session')):
                data = os.listdir(settings.FILE_PATH +'FORM-16')
                print(data)
                return functions.RESPONSE(data,statusCodes.STATUS_SUCCESS)
            elif(requestType.custom_request_type(request.GET, 'get_employees')):
                dept=request.GET['dept'].split(',')
                emp_category=request.GET['emp_category'].split(',')
                data=list(EmployeePrimdetail.objects.filter(dept__in=dept,emp_category__in=emp_category).values('name','emp_id','dept__value','desg__value','emp_status'))
                print(data)
                return functions.RESPONSE(data,statusCodes.STATUS_SUCCESS)
            elif(requestType.custom_request_type(request.GET, 'get_form')):
                emp_id = request.session['hash1']
                name = request.GET['name']
                file_type = request.GET['file_type']
                qry = list(EmployeePerdetail.objects.filter(emp_id=emp_id).values('pan_no'))
                pan_no = qry[0]['pan_no']
                base = name.split('_')[1]
                if(file_type == 'form_A'):
                    filename = "FORM-16/"+name+"/FORM-16_A/"+pan_no+"_"+base+".pdf"
                elif(file_type == 'form_B'):
                    filename = "FORM-16/"+name+"/FORM-16_B/"+pan_no+"_PARTB_"+base+".pdf"
                pdfs = []
                if(os.path.exists(settings.FILE_PATH + filename)):
                    pdfs.append(settings.FILE_PATH + filename)
                else:
                    data = {'msg':"FORM FOR THIS SESSION DOES NOT EXIST"}
                    return functions.RESPONSE(data,statusCodes.STATUS_CONFLICT_WITH_MESSAGE)
                merger = PdfFileMerger()
                for pdf in pdfs:
                    merger.append(open(pdf, 'rb'))
                with open(settings.FILE_PATH + "FORM-16/"+name+"/result.pdf", 'wb') as fout:
                    merger.write(fout)
                with open(settings.FILE_PATH + "FORM-16/"+name+"/result.pdf", 'rb') as pdf:
                    response = HttpResponse(pdf, content_type='application/pdf')
                    response['Content-Disposition'] = 'inline;filename=some_file.pdf'
                    return response
                    pdf.closed
            elif(requestType.custom_request_type(request.GET, 'get_form_hr_end')):
                if (checkpermission(request, [rolesCheck.ROLE_HR]) == statusCodes.STATUS_SUCCESS):
                    ...
                    return functions.RESPONSE(data,statusCodes.STATUS_UNAUTHORIZED)
...
################### For FORM-16 it should have a particular file name format in the Directory of FORM-16 ###################
