	...
	if checkpermission(request, [rolesCheck.ROLE_ACADEMIC]) == statusCodes.STATUS_SUCCESS:
		...
		if requestMethod.GET_REQUEST(request):
			if(requestType.custom_request_type(request.GET, 'subjects')):
				...
				overall_y = 0
				overall_n = 0
				if 'primary_branch' in request.GET :
					student_list = get_section_students(request.GET['section'].split(","), {"uniq_id__dept_detail__dept_id__in":request.GET['primary_branch'].split(',')}, session_name)
				else:
				...
				co_info = list(SubjectCODetails.objects.filter(subject_id=subject_id).exclude(status='DELETE').exclude(co_num= -1).values('id').annotate(co_name=F('co_num'), co_description=F('description')))
				for d in co_info:
					nam = 'CO'+str(d['co_name'])
					co_wise_list[nam] = {}
					co_wise_list[nam]['Y'] = 0
					co_wise_list[nam]['N'] = 0
				# print(co_info)
						...
						if co_info: 
							...
							for c in co_info:
								...
								for k,v in c.items():
									t[k] = v
								#################################
								# print(c,"co_information")
								get = 0
								y = 0
								no1 = 0
								na = 0
								nam1 = 'CO'+str(c['co_name'])
								if c['co_name'] > 0:
									qid = []
									print(paper_id[0]['ques_paper_id'],"paper_id[0]['ques_paper_id']")
									print(c['id'],"ques_id__co_id=c['id']")
									...
									print(co_section_details,"co_section_details")
									...
									if co_section_details:
										...
										###### CHANGE BY VRINDA ######
										t['co_max_marks'] = co_max_marks.get('co_max_marks', 0)
										co_info_data.append(t)
										##############################
										# if(e['sno'] == 201):
											# print(qid, c['co_name'], c['id'], c['co_max_marks'])

										for stu in students:
											# print(stu,"student_uniq_id")
								...
								if get == 1:
									...
									no.append(no1)
									na_array.append(na)
									co_wise_list[nam1]['Y']+=y
									co_wise_list[nam1]['N']+=no1
									overall_y+=y
									overall_n+=no1
				...
				data = {'data': exam_data, 'data2': students, 'yes_count': yes, 'no_count': no, 'na_count': na_array, 'university_marks_entered': university_marks_entered, 'attainment_level': attainment_level,'co_wise':co_wise_list,'overall_n':overall_n,'overall_y':overall_y}
				...
				data = {'data': exam_data, 'data2': students, 'yes_count': yes, 'no_count': no, 'na_count': na_array, 'university_marks_entered': university_marks_entered, 'attainment_level': attainment_level}
			elif (requestType.custom_request_type(request.GET, 'get_report_data_lab_new_dept_wise')):
					subject_id = request.GET['subject_id']
					sem_id = request.GET['sem']
					exam_data = []
					exam_list = get_exam_name(session)
					...
					if request.GET['isgroup'] == 'N':
						...
						group = request.GET['group_id'].split(",")
						student_list = []
						dept=request.GET['primary_branch'].split(',')
						for g in group:
							student = get_att_group_students_nominal_dept_wise(g, session_name,dept)
	...
	if checkpermission(request,[rolesCheck.ROLE_ACADEMIC]) == statusCodes.STATUS_SUCCESS:
		...
		if requestMethod.GET_REQUEST(request):
			if(requestType.custom_request_type(request.GET,'subjects')):
				...
				# exam_pid=list(StudentAcademicsDropdown.objects.filter(field='LAB MANUAL',session=session).exclude(value__isnull=True).values('value','sno','pid'))

				# for e in exam_list:
				#     paper_id=list(AssignmentStudentMarks.objects.filter(marks_id__exam_id=e['sno'],marks_id__subject_id=subject_id).exclude(status='DELETE').values('id','marks_id__isco_wise','marks_id__max_marks','ques_id__ques_paper_id'))
					
				#     if paper_id:
				#         if paper_id[0]['marks_id__isco_wise']=='N':   
				#             co_info=list(SubjectCODetails.objects.filter(subject_id=subject_id).exclude(status='DELETE').values('id').annotate(co_name=F('co_num'),co_description=F('description')))

				#             qry=list(SubjectCODetailsAttainment.objects.exclude(status='DELETE').filter(co_id__subject_id=subject_id,attainment_method=paper_id[0]['pid']).values('id').annotate(co_req_attain=F('attainment_per')))
				...
				#                             stu['marks'].append(obtained_co_marks)
				#                             attainment_per_obtained = (obtained_co_marks/paper_id[0]['marks_id__max_marks'])*100.0
				...
				#             for c in co_info:
				#                 qry=list(SubjectCODetailsAttainment.objects.filter(co_id=c['id'],attainment_method__value='LAB MANUAL').values('id').annotate(co_req_attain=F('attainment_per')))
	...
	if checkpermission(request, [rolesCheck.ROLE_DEAN]) == statusCodes.STATUS_SUCCESS:
		...
		if requestMethod.GET_REQUEST(request):
			if(requestType.custom_request_type(request.GET, 'get_attainment_limit')):
				...
				qry = MarksAttainmentSettings.objects.filter(subject_type__in=sub_type, sem__dept__in=dept, sem__sem__in=sem, attainment_type='D').exclude(status="DELETE").values('from_direct_per', 'to_indirect_per', 'attainment_level', 'subject_type__value').annotate(dept=F('sem__dept__dept__value'), course=F('sem__dept__course__value'), sem=F('sem__sem'))
				qry2 = MarksAttainmentSettings.objects.filter(subject_type__in=sub_type, sem__dept__in=dept, sem__sem__in=sem, attainment_type='A').exclude(status="DELETE").values('external_marks', 'internal_marks', 'from_direct_per', 'to_indirect_per', 'subject_type__value').annotate(dept=F('sem__dept__dept__value'), course=F('sem__dept__course__value'), sem=F('sem__sem'))
	...
	if checkpermission(request, [rolesCheck.ROLE_ACADEMIC]) == statusCodes.STATUS_SUCCESS:
		...
		if requestMethod.GET_REQUEST(request):
			...
			for sec in sections:
				...
				qry_marks_id = AssignmentQuizMarks.objects.filter(section=section_id, subject_id=subject_id, exam_id=exam_id, isgroup=isgroup, group_id=group_id).exclude(status='DELETE').values('id', 'isco_wise', 'max_marks').order_by('-id')
				final_data = {}
			...
			#     else:
			#         group_marks=group_marks[0]['id']
			#         group_marks=AssignmentQuizMarks.objects.exclude(status='DELETE').filter(id=group_marks).update(**qry_create)
			#     if group['isco_wise']=='Y':
			#         for student in group['students']:
			#             qry=AssignmentStudentMarks.objects.filter(marks_id__exam_id=group['exam_id'],marks_id__subject_id=group['subject_id'],uniq_id=student['uniq_id']).update(status='DELETE')
			#             set_index=student['set_index']
			...
			if(requestType.custom_request_type(request.GET, 'get_batch')):
				...
				for name in batch_session:
					...
					for subject in subject_id:
						if 'secondary_branch' in request.GET:
							subject_data = subject_overall_attainment_dept_wise(subject['id'], subject['subject_type'], subject['sem'], name, session[0]['uid'],request.GET['secondary_branch'].split(','))
						else:
	...
	if checkpermission(request, [rolesCheck.ROLE_HOD, rolesCheck.ROLE_DEAN]) == statusCodes.STATUS_SUCCESS or academicCoordCheck.isNBACoordinator(request) == True:
		...
		if(requestType.custom_request_type(request.GET, 'form_data')):
			...
			for name in batch_session:
				...
				sem = (2 * (batch_till - batch_from)) + plus_factor
				# extra_filter = {}
				# print(type(sem), type(dept))
				# if dept == '44' and sem == 7:
				#     print('1')
				#     extra_filter = {'id': 1285}
				...
				for subject in subject_id:
					...
					for po in po_list:
						if 'secondary_branch' in request.GET:
							subject_overall_attain = get_subject_po_attainment_dept_wise(subject['id'], subject['subject_type'], po['id'], subject['sem'], name, session[0]['uid'],request.GET['secondary_branch'].split(','))
						else:
	...
	if checkpermission(request, [rolesCheck.ROLE_HOD, rolesCheck.ROLE_DEAN]) == statusCodes.STATUS_SUCCESS or academicCoordCheck.isNBACoordinator(request) == True:
		if(requestType.custom_request_type(request.GET, 'form_data')):
			...
			for name in batch_session:
				...
				sem = (2 * (batch_till - batch_from)) + plus_factor
				qry = list(SubjectInfo.objects.filter(sem__sem=sem, sem__dept=dept, session=session[0]['uid']).exclude(status='DELETE').values('sem'))
				if len(qry) > 0:
					sem_id = qry[0]['sem']
				subject_id = list(SubjectInfo.objects.filter(sem__sem=sem, sem__dept=dept, session=session[0]['uid']).exclude(status='DELETE').values('id', 'subject_type__value', 'sem', 'subject_type', 'sub_alpha_code', 'sub_num_code', 'sub_name'))
				...
				for subject in subject_id:
					...
					for po in po_list:
						if 'primary_branch' in request.GET:
							subject_overall_attain = get_subject_po_attainment_dept_wise(subject['id'], subject['subject_type'], po['id'], subject['sem'], name, session[0]['uid'],request.GET['primary_branch'].split(','))
						else:
							subject_overall_attain = get_subject_po_attainment(subject['id'], subject['subject_type'], po['id'], subject['sem'], name, session[0]['uid'])
						subject['po_data'].append({'po_id': po['id'], 'po_num': po['po_level_abbr'], 'po_desc': po['description'], 'sub_po_attain': subject_overall_attain})
					subject_array.append(subject)
