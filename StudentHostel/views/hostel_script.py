...
def Allot_Room(emp_id, session, session_name, hostel_id):
    ...
    bulk_query = (HostelRoomAlloted(rule_used=HostelRoomAllotSetting.objects.get(id = student['rule_used']), uniq_id=studentSession.objects.get(uniq_id = student['uniq_id']), room_part=HostelRoomSettings.objects.get(id = student['room_part']), status="TEMP") for student in student_alloted)
