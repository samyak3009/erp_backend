...
def Room_Settings(request):
    ...
    if checkpermission(request, [rolesCheck.ROLE_CHIEF_RECTOR_GIRLS, rolesCheck.ROLE_CHIEF_RECTOR_BOYS]) == statusCodes.STATUS_SUCCESS or academicCoordCheck.isRector(request) == True:
        ...
        if (requestMethod.GET_REQUEST(request)):
            ...
            check_deleted_room_is_alloted=HostelRoomAlloted.objects.filter(room_part__in=list(room_ids_to_be_delete)).exclude(status="DELETE").exists()
            if(check_deleted_room_is_alloted):
                return functions.RESPONSE({"msg":"Room Cannot Be Deleted As It Has Been Already Alloted"}, statusCodes.STATUS_CONFLICT_WITH_MESSAGE)
    ...
    if checkpermission(request, [rolesCheck.ROLE_CHIEF_RECTOR_GIRLS, rolesCheck.ROLE_CHIEF_RECTOR_BOYS]) == statusCodes.STATUS_SUCCESS or academicCoordCheck.isRector(request) == True or academicCoordCheck.isWarden(request) == True:
        ...
        if (requestMethod.GET_REQUEST(request)):
            ...
            ####SEAT ALLOTED STUDENTS LIST###FOR HOSTEL SWAP######BY YASH###################
                ...
                data = list(HostelSeatAlloted.objects.filter(hostel_part=hostel_id,seat_part__in=bed_capacity,paid_status="ALREADY PAID").exclude(status='DELETE').values('uniq_id','uniq_id__uniq_id__name'))
                for d in data:
            ...
            bulk_create_seat=(HostelSeatAlloted(uniq_id=studentSession.objects.get(uniq_id=r['uniq_id']),hostel_part=HostelDropdown.objects.get(sno=r['hostel_part']),seat_part=HostelDropdown.objects.get(sno=r['seat_part']),paid_status='ALREADY PAID')for r in query_list_seat)
