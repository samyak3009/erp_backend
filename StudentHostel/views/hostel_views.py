...
def getComponents(request):
    if checkpermission(request, [rolesCheck.ROLE_CHIEF_RECTOR_GIRLS, rolesCheck.ROLE_CHIEF_RECTOR_BOYS, rolesCheck.ROLE_STUDENT_ACCOUNTS,rolesCheck.ROLE_LIBRARY_REPORT,rolesCheck.ROLE_HOD]) == statusCodes.STATUS_SUCCESS or academicCoordCheck.isRector(request) == True:
        emp_id = request.session['hash1']
    ...
    if checkpermission(request, [rolesCheck.ROLE_CHIEF_RECTOR_BOYS, rolesCheck.ROLE_CHIEF_RECTOR_GIRLS]) == statusCodes.STATUS_SUCCESS or academicCoordCheck.isRector(request) == True:
        ...
        if requestMethod.POST_REQUEST(request):
            ...
            if 'allotment_status' in data:
                ...
                if allotment_status != "NOT ALLOTED":
                    HostelSeatAlloted.objects.filter(uniq_id__in=list(uniq_id_list)).exclude(status="DELETE").update(status='DELETE')
                    room_part_allotted_status=list(HostelRoomAlloted.objects.filter(uniq_id__in=list(uniq_id_list)).exclude(status="DELETE").values_list('room_part__allotted_status','room_part'))
                    print(room_part_allotted_status[0][0])
                    print(room_part_allotted_status[0][1])
                    HostelRoomSettings.objects.filter(id=room_part_allotted_status[0][1]).exclude(status="DELETE").update(allotted_status=room_part_allotted_status[0][0]-1)
                    HostelRoomAlloted.objects.filter(uniq_id__in=list(uniq_id_list)).exclude(status="DELETE").update(status='DELETE')
            ...
            if 'bed_capacity' in data:
                ...
                qry=HostelRoomAlloted.objects.filter(uniq_id=data['uniq_id']).exclude(status="DELETE").exclude(room_part__status="DELETE").values('room_part__allotted_status')
                print(qry[0]['room_part__allotted_status'])
                if len(qry)==0 or str(qry[0]['room_part__allotted_status'])==data['bed_capacity']:
                    ...
                    data=statusMessages.CUSTOM_MESSAGE('PLEASE UN-ALLOT THE ROOM FIRST')
                    return functions.RESPONSE(data,statusCodes.STATUS_CONFLICT_WITH_MESSAGE)
