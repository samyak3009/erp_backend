                ...
                if request.method == 'POST':
                    ...
                    if inp['leave'] == '2':
                        ...
                        if 'remark' in inp:
                            ...
                            upd_list={'status':status,'intime':intime,'outtime':outtime,'colstatus':'green','extra':extra,'remark':inp['remark']}
                            print(upd_list)

                        else:  
                            upd_list={'status':status,'intime':intime,'outtime':outtime,'colstatus':'green','extra':extra}
            ...
            if(qry3[0]['shift__field']=='FLEXIBLE'):
                ...
                if(qry0.count()==0): ## if no punch than absent(A) status
                    ...
                    if(qry_in[0]['time']<qry4[0]['intime'] and qry_out[0]['time']<qry4[0]['intime']): ### if punch IN AND PUNCH out time is less Than In shift in Time
                        ...
                        print(employee_intime,qry4[0]['intime'],employee_outtime,qry4[0]['outtime'])
                        time=datetime.timedelta(seconds=sec)
                        if(sec<0):
                            status='A'
                            colstatus='red'
...
def attendance_script(request):
    ...
    if check11 == True:
        ...
        for qry_emp1 in qry_emp:
            ...
            qry4=Shifts.objects.filter(shiftid=qry3[0]['shift']).values() #get shidt details
            if(qry4.count()!=0):
