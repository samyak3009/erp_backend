...
from StudentAccounts.models import SubmitFee,ModeOfPaymentDetails,SubmitFeeComponentDetails,StuAccFeeSettings,RefundFee
...
def get_profile(uniq_id,session_name):
	...
	q_father_mob = StudentFamilyDetails.objects.filter(uniq_id=uniq_id).values('father_mob','mother_mob','father_occupation','mother_occupation','mother_uan_no','mother_uan_no','father_uan_no')
		...
		if request.user.is_authenticated:
			if request.session['hash3'] == 'Student':
				...
				if(request.method=='GET'):
					...
					q_check = SubmitFee.objects.filter(fee_rec_no=fee_rec_no).values('fee_rec_no','uniq_id__name','cancelled_status','actual_fee','paid_fee','refund_value','due_value','session__session','receipt_type','id','time_stamp','uniq_id','receipt_type','prev_fee_rec_no','uniq_id__gender__value')
					if len(q_check)==0:
						status=403
			...
			if request.session['hash3'] == 'Student':
				...
				if(request.method=='GET'):
					...
					if(request.GET['request_type']=='initial_data'):
						...
						data_values={'stu_det':list(stu_det),'personal_details':list(personal_details),'insurance_details':list(insurance_details),'bank_details':list(bank_details),'family_details':list(family_details)}
						status=200
...
def printDeclaration(request):
	...
	uniq_id = 319
	session_name = '1819o'
	Declarationpdf(uniq_id,session_name)
	with open(settings.FILE_PATH+"declaration.pdf", 'rb') as pdf:
		response = HttpResponse(pdf, content_type='application/pdf')
		response['Content-Disposition'] = 'inline;filename=some_file.pdf'
				...
				if request.method=='GET':
					if (request.GET['request_type']== 'form_data'):
						...
						if len(course_dur)==0:
							...
							# details[0]['msg']='Portal is locked'
							if len(qry)==0:
								data_values = {'msg':'Hostel is not alloted.'}
								status = 200
							else:
								...
								if locking_status==False:
									...
									if len(qry1)!=0:
										...
										data_values = {'msg':'You have already filled your roommate choice form','app_id':app_id[0],'allready_filled_data':already_filled_data,'data':list(query)}
										status = 200
									else:
										...
										#   status = 200
											...
											if qry[0]['paid_status']!='ALREADY PAID':
												...
												data_values = {'data':list(query),'seater':qry[0]['seat_part__value']}
												status = 200
					# elif (request.GET['request_type']== 'edit_form'):
					#   app_id = request.GET['app_id']
...
def get_stu_data(uniq_id,session_name):
	studentSession = generate_session_table_name('studentSession_',session_name)
	HostelSeatAlloted = generate_session_table_name('HostelSeatAlloted_',session_name)
	data = list(studentSession.objects.filter(uniq_id=uniq_id).values('uniq_id__name','uniq_id__uni_roll_no','uniq_id__dept_detail__course__value','uniq_id__dept_detail__course','uniq_id__batch_from','uniq_id__caste','uniq_id__gender','uniq_id__fee_waiver','uniq_id__admission_status','uniq_id__dept_detail__dept__value','year'))
	...
	if len(data)>0:
		for d in data:
			...
			academic_fee_data = StuAccFeeSettings.objects.filter(
				join_year=data[0]['uniq_id__batch_from'],
			...
			for acad in academic_fee_data:
				if 'UPSEE' in acad:
					if ((data[0]['year'] == 2 and 'LATERAL' in data[0]['uniq_id__admission_type__value']) or (data[0]['year'] == 1)) and (data[0]['uniq_id__admission_through__value'] == 'COUNSELLING') and (data[0]['uniq_id__join_year']==date.today().year):
						flag_upsee=1
			...
			if len(trans_data)==0:
				...
				for bus in trans_data:
					if bus['fee_component_cat__value']=='BUS FEE GHAZIABAD':
						d['ghaz'] = bus['value']
			...
			if request.session['hash3'] == 'Student':
				...
				if request.method == 'GET':
					if (request.GET['request_type'] == 'print_form'):
						form_type = PlacementPolicy.objects.filter(uniq_id = uniq_id).exclude(status = "DELETE").values_list('form_type',flat = True)
						...
						course_duration=list(studentSession_1920o.objects.filter(uniq_id=uniq_id).values('sem__dept__course_duration','sem'))

						if course_duration:
							year=course_duration[0]['sem__dept__course_duration']
							second_last = list(studentSession.objects.filter(year=year,uniq_id=uniq_id,uniq_id__dept_detail__course=12).values('uniq_id'))
						else:
							second_last=[]

						if second_last:
							if date < '2019-05-30' and date > '2019-05-25':
								Placement_id = PlacementPolicy.objects.filter(uniq_id = uniq_id).exclude(status = "DELETE").values_list('id',flat = True)
								if len(Placement_id) == 0:
									link = 'https://tech.kiet.edu/Placement_Policy_2020_B.Tech_Updated.pdf'
									details = studenthostel.views.hostel_function.get_student_details(uniq_id,session_name,{},sem_type,session_id)
									data_values = {'data':details,'link':link,'already_filled':False}
									status = 200

								else:
									form_type = PlacementPolicy.objects.filter(uniq_id = uniq_id).exclude(status = "DELETE").values('form_type','time_stamp')
									if len(form_type)>0:
					...
					PlacementPolicy.objects.create(uniq_id = studentSession_1819e.objects.get(uniq_id = uniq_id), form_type = form_type)
...
def printPlacementForm(uniq_id,form_type,session_name):
	...
	sno=list(PlacementPolicy.objects.filter(uniq_id=uniq_id).values('id'))
...
def AcademicFeeLetter(stu_data,filename):
	...
	for x in stu_data:
		...
		draw.text((700,630+w),str(': '+Comma_function(str(stu_data[x]))+'/-'),(0,0,0),font=font_bold)
		w=w+20
	...
	draw.text((150,920),str('-------------------------------------------------------------------------------------------'),(0,0,0),font=font_bold)
	draw.text((250,940),str('Total'),(0,0,0),font=font_bold)
	draw.text((700,940),str(': '+Comma_function(str(stu_data['total']))+'/-'),(0,0,0),font=font_bold)
