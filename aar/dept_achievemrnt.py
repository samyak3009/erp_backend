                    ...
                    if request.GET['check']=='GUIDANCE':
                        ...
                        if request.GET['check2']=='3':
                            if request.GET['data']=='70':
                                emp_id=request.session['hash1']
                                a=[]
                                sno=list(AarDropdown.objects.filter(field='RESEARCH GUIDANCE / PROJECT GUIDANCE',value='COURSES').exclude(value=None).values('sno'))
                                s1=AarDropdown.objects.filter(pid=sno[0]['sno']).exclude(value=None).values('value','sno')
                            ...
                            if request.GET['data']=='71':
                                emp_id=request.session['hash1']
                                a=[]
                                sno=list(AarDropdown.objects.filter(field='RESEARCH GUIDANCE / PROJECT GUIDANCE',value='DEGREE').exclude(value=None).values('sno'))
                                s1=AarDropdown.objects.filter(pid=sno[0]['sno']).exclude(value=None).values('value','sno')
                                sno1=list(AarDropdown.objects.filter(field='RESEARCH GUIDANCE / PROJECT GUIDANCE',value='UNIVERSITY TYPE').exclude(value=None).values('sno'))
                                s3=AarDropdown.objects.filter(pid=sno1[0]['sno']).exclude(value=None).values('value','sno')
                                s2=Researchguidence.objects.filter(emp_id=emp_id,guidence=request.GET['data']).values('date','emp_id','guidence__value','course__value','degree__value','no_of_students','degree_awarded','status__value','project_title','area_of_spec','uni_name','uni_type__value','id','uni_type')
                    ...
                    if request.GET['check']=='GUIDANCE':
                        ...
                        if request.GET['check2']=='true':
                            if request.GET['data']=='70':
                                a=[]
                                sno=list(AarDropdown.objects.filter(field='RESEARCH GUIDANCE / PROJECT Guidance',value='COURSES').exclude(value=None).values('sno'))
                                s=AarDropdown.objects.filter(pid=sno[0]['sno']).exclude(value=None).values('value','sno')
                                data_values=list(s)
                                status=200
                            if request.GET['data']=='71':
                                a=[]
                                ...
                                sno1=list(AarDropdown.objects.filter(field='RESEARCH GUIDANCE / PROJECT GUIDANCE',value='UNIVERSITY TYPE').exclude(value=None).values('sno'))
                                s=AarDropdown.objects.filter(pid=sno[0]['sno']).exclude(value=None).values('value','sno')
                                s1=AarDropdown.objects.filter(pid=sno1[0]['sno']).exclude(value=None).values('value','sno')
                                data_values={'data':list(s),'data1':list(s1)}
                                status=200
                            if request.GET['data']=='72':
                                a=[]
                                sno=list(AarDropdown.objects.filter(field='GUIDANCE',value='RESEARCH (PH. D.)').exclude(value=None).values('sno'))
                                sno1=list(AarDropdown.objects.filter(field='RESEARCH GUIDANCE / PROJECT GUIDANCE',value='UNIVERSITY TYPE').exclude(value=None).values('sno'))
                                s=AarDropdown.objects.filter(pid=sno[0]['sno']).exclude(value=None).values('value','sno')
                                s1=AarDropdown.objects.filter(pid=sno1[0]['sno']).exclude(value=None).values('value','sno')
                                data_values={'data':list(s),'data1':list(s1)}
                    ...
                    if(data['test']=='1'):
                        ...
                        if(sno=='1'):
                            ...
                            for emp in emps:
                                ...
                                if(q.count()>0):
                                    for i in q:
                                        qry = Sponsers.objects.filter(spons_id=i['id'],type="CONFERENCE").values('sponser_name')
                                        i['SPONSER']=list(qry)
                            ...
                            for emp in emps:
                                ...
                                if(q.count()>0):
                                    for i in q:
                                        qry2 = Discipline.objects.filter(id=i['id'],type="PATENT").values('value1__value')
                                        for s in qry2:
                                            if s['value1__value']==None:
                                                s['value1__value']=EmployeePrimdetail.objects.filter(emp_id=i['emp_id']).values('dept__value')
                                        i['discipline'] = list(qry2)
                            ...
                            for emp in emps:
                                ...
                                for i in q:
                                    qry = Sponsers.objects.filter(spons_id=i['id'],type="TRAINING").values('emp_id','sponser_name','type','pin_code','address','contact_person','contact_number','e_mail','website','amount','field_type')
                                    qry2 = Discipline.objects.filter(id=i['id'],type="TRAINING").values('value1__value')
                                    i['COLLABORATION']=[]
                                    i['SPONSOR']=[]
                                    i['discipline']=list(qry2)
                                    for s in qry:
                                        if s['field_type']=='COLLABORATION':
                                            i['COLLABORATION'].append(s)
                                        else:
                                            i['SPONSOR'].append(s)
                                    b.append(i)
                        ...
                        if(data['dept']=="all"):
                            ...
                            if(query_data.count()>0):
                                for i in query_data:
                                    qry2 = Discipline.objects.filter(id=i['id'],type="PATENT").values('value1__value')
                                    for s in qry2:
                                        if s['value1__value']==None and 'emp_id' in i:
                                            s['value1__value']=EmployeePrimdetail.objects.filter(emp_id=i['emp_id']).values('dept__value')
                                    i['discipline'] = list(qry2)
                        ...
                        if(data['dept']=="all"):
                            ...
                            if(query_data.count()>0):
                                for i in query_data:
                                    qry = Sponsers.objects.filter(spons_id=i['id'],type="PROJECT").values('sponser_name','type','pin_code','address','contact_person','contact_number','e_mail','website','amount','field_type')
                                    qry2 = Discipline.objects.filter(id=i['id'],type="PROJECT/CONSULTANCY").values('value1__value')
                                    i['grant_total'] = Sponsers.objects.filter(spons_id=i['id'],type="PROJECT").extra(select={'grant_total':'SUM(amount)'}).values('grant_total')[0]['grant_total']
                                    print(i['grant_total'])
                                    i['ASSOCIATION']=[]
                                    i['SPONSOR']=[]
                                    i['discipline']=list(qry2)
            ...
            if(check == 200):
                ...
                if(request.method=='POST'):
                    ...
                    if(data['type']=='1'):
                        ...
                        retrieve_data=[]
                        query_data=Patent.objects.filter(emp_id=emp_id,date__lte=tdate,date__gte=fdate).exclude(approve_status='PENDING').exclude(approve_status='DELETE').values('emp_id__name','title','id','emp_id','approve_status','descreption','collaboration','company_name','incorporate_status__value','status','date','owner__value','emp_id__dept__value','number')
                        if(query_data.count()>0):
                            j=0
                            for i in query_data:
                                qry2 = Discipline.objects.filter(id=i['id'],type="PATENT").values('value1__value')
